...
export function GlobalSettings({ actions, notify }: { actions: Map<string, ActionEntry>; notify: (message: string, type?: 'success' | 'error') => void }) {
  ...
  const load = useCallback(async () => { const spec = actions.get('settings_global:read'); if (!spec) return; try { const data: any = await request(spec.path, { method: spec.method }); setItem(data.item || data); } catch (error) { notify((error as Error).message, 'error'); } }, [actions, notify]);
...
export function BanList({ actions, notify }: { actions: Map<string, ActionEntry>; notify: (message: string, type?: 'success' | 'error') => void }) {
  ...
  const load = useCallback(async () => { const spec = actions.get('security_bans:list'); if (!spec) return; try { const data: any = await request(spec.path, { method: spec.method }); const rows = Array.isArray(data) ? data : data.items || data.rows || []; setItems(rows.map((item: AnyRecord) => ({ ...item, key: item.key ?? item.Key, ban_type: item.ban_type ?? item.BanType, fail_times: item.fail_times ?? item.FailTimes, is_banned: item.is_banned ?? item.IsBanned, last_login_time: item.last_login_time ?? item.LastLoginTime }))); } catch (error) { notify((error as Error).message, 'error'); } }, [actions, notify]);
  useEffect(() => { void load(); }, [load]);
