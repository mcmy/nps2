...
- 添加隧道限速功能
- 修复暗黑模式下验证码无法识别
- 设备详情页支持按协议展开连接命令并一键复制
- 首页接入端点显示主桥接地址
- 修复新建/复制资源时到期时间继承和空值显示异常
- 修复新建资源时客户端默认选中异常

### v0.35.0 (2026-08-07)
- 迁移为 React/Astro 管理台，支持浅色/深色主题、中英文切换和响应式布局
- 接入当前版本已有的客户端、隧道和域名代理管理接口
- 增加管理台能力发现接口，兼容旧版 Beego 登录和资源接口
- 清理旧版 Beego 模板、Bootstrap 资源和旧前端页面
- 管理台静态资源增加版本缓存标识，避免浏览器继续使用旧 bundle
- 更新 GitHub Actions 发布流程，区分 `v*` 标签正式发布和 `dev/v*` 分支预发布
